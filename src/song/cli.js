...
function parseArgs(argv) {
  const args = { provider: 'mock', seconds: 2, out: undefined, prompt: '' };
  const rest = [];
...
async function main() {
  ...
  if (!args.prompt) {
    console.error('Usage: node src/song/cli.js "<prompt>" [--provider mock|replicate|local] [--seconds N] [--out DIR]');
    process.exit(1);
  }
